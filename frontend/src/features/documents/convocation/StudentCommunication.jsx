...
import localData from './temporaryData'
import { toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import config from '../../../config.js'
import { emailPayloadSchema } from './schema'



const StudentCommunication = ({ student }) => {
  ...
  const manageRadioInputValues = (e) => {
    ...
    const manageFetchData = (e) => {
      ...
      let fetchData = {
        ...
        emailType: 'convocationFormation',
        studentData: {
      ...
      if (inputValue === "convoc-permis") {
        fetchData.emailType = "convocationExam"
      }
      if (showDocumentOption === "none") {
        delete fetchData.fileData
      }

      const validation = emailPayloadSchema.safeParse(fetchData)
      ...
      .then(async response => {
        ...
        if (data.emailIsArrived === true) {
          notifyIfEmailIsArrived(data.toastNotification, data.datetime)
        }
      })
      .catch(error => {
        console.error(error)
    ...
    <section className="mb-3 container-md d-flex flex-column justify-content-between border border-lightgrey border-1 rounded overflow-hidden">
      ...
      {showDocumentOption === "block" && (
        ...
        <div className="container-md d-flex justify-content-center align-items-center document-viewver bg-secondary bg-opacity-25 mb-3">
          ...
          <ConvocationForm student={student} setFormFetchData={setFormFetchData} />
        </div>
      </div>
      )}
    </section>
